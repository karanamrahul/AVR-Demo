...
from src.models.license_plate_recognition.alpr import ALPR
...
from src.utils.general import ROOT, add_image_id,convert_to_deepsort_format
...
class FileProcessThread(QThread):
    ...
    def set_start_config(self, video_path, ai_task, screen_size, model_name="yolov8n", tracker_name="deepsort", confidence_threshold=0.35, iou_threshold=0.45, frame_interval=0):
        ...
        self._init_yolo()
        self._init_tracker()
        self.license_detector = ALPR(out_dir=os.path.join(ROOT, "src/models/license_plate_recognition/out"))
    ...
    def run(self):
        ...
        while self.threadFlag:
            ...
            frame = draw_results(frame, model_output,self.ai_task)
